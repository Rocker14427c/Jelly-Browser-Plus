...
|---|---|
| **v16.23** | **Lightweight pass**: the three nested ad-block lists are merged into one level-tagged file (smaller APK, faster load); only English + Hindi strings are packaged (slims resources.arsc); history is capped at the newest 2000 entries so the database (favicons included) stops growing forever; Settings gains **Clear browsing cache** which frees WebView cache space across all tabs. |
| **v16.22** | **Find-in-page counts what you can see**: text inside hidden sections (collapsed accordions, display:none overlays, off-screen carousels) is no longer counted or highlighted; the current match is force-centered with a verification pass (inner scroll containers can no longer swallow the scroll), so the highlighted match is always on screen. **History & favorites now show the site's web icon**: favicons are stored (64px PNG) in both databases (v3→v4 migrations) and displayed in the row circles/card banners with the initial as fallback; icons are captured on page load and when bookmarking. |
| **v16.21** | **History now records every visit**: pages are recorded on page-finished (not just when a title arrives), so SPAs and sites opened via favorites always appear; revisits move back to the top. **Find-in-page rewritten**: matches are wrapped in visible yellow highlights with an outlined current match and smooth scroll-into-view (works in dark mode too); the native fallback remains for pages with JS disabled. **Resume freeze fixed**: the active WebView is no longer detached/re-attached on every onStart — returning from Recents on heavy pages (AI chats) resumes instantly instead of stalling for up to a minute; dark-mode injection and client rebinds are skipped when nothing changed. |
| **v16.20** | **Favorites redesigned** (Chrome-bookmarks style): colorful banner cards with a big site-initial circle, title and host, plus a toolbar **search**. **Downloads multi-select**: a selection icon in the toolbar enters selection mode (checks on rows, tap/long-press toggles), with a delete action that asks for confirmation. **Element picker redone properly**: tapping now only highlights the selection (red overlay box) and opens an action sheet — Block element / Mark host as ad / **Expand selection** (grows to the parent, from tiny elements up to the whole page) / Cancel — and blocking asks for confirmation first. The blocked-rules manager shows separate Hosts/Elements sections with per-rule remove buttons. Cookie entries removed from the lock menu (already in Settings). |
...
| **v16.15** | Long-press a download → **Rename / Copy download link / Delete** (red) only; the ⋮ button is gone. **Start page**: blank tabs show Favorites / Downloads / History quick links instead of an empty page. **Chrome-style tab button**: a rounded pill showing the number of open tabs. |
...
- **Tabs** — the Chrome-style tabs pill (shows the number of open tabs) opens the Via-style
  switcher: tap a card to
...
- **Downloads** — **⋮ → Downloads** opens the in-app download manager: progress, per-download
  speed, pause/resume, cancel, and tap a finished file to open it. **Long-press a download** for
  Rename / Copy download link / Delete (red). The download snackbar has a **See** action that
  jumps straight to Downloads. Downloads keep running in the background with a notification
  while anything is active; a download interrupted by the app being killed shows up as paused
  and can be resumed.
- **Start page** — new/blank tabs show quick links to **Favorites, Downloads and History**
  instead of an empty page; the page disappears the moment you navigate.
